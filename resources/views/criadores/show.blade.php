...
@section('title', 'Criadores - Show')

@section('content_header')
    <h1>Criador {{$breeder->id}}</h1>
@stop
...
@section('content')
    ...
    <div class="card">
        <div class="card-header text-right">
            <a href="{{route('criadores.create')}}" class="btn btn-primary flex-wrap">
                <i class="fas fa-plus"></i>
                Nuevo
            </a>
            <a href="{{route('criadores.edit', $breeder)}}" class="btn btn-primary flex-wrap">
        ...
        <div class="card-body">
            <div class="row">
                <div class="col-12 col-sm-4 text-center">
                    <img src="{{ asset('img/breeders/no_image_500.png') }}" alt="user-avatar" class="img-circle img-fluid" style="max-width: 300px;">
                </div>
                <div class="col-12 col-sm-8">
                    <h3 class="my-3">{{$breeder->name}}</h3>
                    <div class="table-responsive">
                        <table class="table">
                            <tbody>
                                <tr><th>Código:</th><td>{{$breeder->code}}</td></tr>
                                <tr>
                                    <th>Teléfono:</th>
                                    <td>
                                        {{$breeder->tel_01 ?? ''}}
                                        @if($breeder->tel_02 != null)
                                            <br>
                                            {{$breeder->tel_02}}
                                        @endif
                                    </td>
                                </tr>
                                <tr><th>Email:</th><td>{{$breeder->mail}}</td></tr>
                                <tr>
                                    <th style="width:25%">Dirección:</th>
                                    <td>
                                    {{$breeder->address->street ?? ''}} {{$breeder->address->ext_number ?? ''}}
                                    @if($breeder->address->int_number != null)
                                        {{"Interior. "}}{{$breeder->address->int_number}}
                                    @endif
                                    <br>
                                    @if($breeder->address->col != null)
                                        {{"Col. "}}{{$breeder->address->col}}
                                    @endif
                                    <br>
                                    @if($breeder->address->city != null)
                                        {{$breeder->address->city}}{{", "}}
                                    @endif
                                    @if($breeder->address->mun != null)
                                        {{$breeder->address->mun}}{{", "}}
                                    @endif
                                    @if($breeder->address->state != null)
                                        {{$breeder->address->state}}{{", "}}
                                    @endif
                                    @if($breeder->address->country != null)
                                        {{$breeder->address->country}}
                                    @endif
                                    @if($breeder->address->zip_code != null)
                                        <br>
                                        {{"CP. "}}{{$breeder->address->zip_code}}
                                    @endif
                                    </td>
                                </tr>
                                <tr><th>Observaciones:</th><td>{{$breeder->notes}}</td></tr>
                                <tr><th>Creación:</th><td>{{$breeder->created_at}}</td></tr>
                                <tr><th>Actualización:</th><td>{{$breeder->updated_at}}</td></tr>
                            </tbody>
                        </table>
                    </div>
                </div>
                    ...
                    <div class="card card-primary card-tabs">
                        <div class="card-header p-0 pt-1">
                            <ul class="nav nav-tabs" id="locations-tab" role="tablist">
                                <li class="nav-item">
                                    <a class="nav-link active" id="breeders-specimens-tab" data-toggle="pill" href="#specimens-tab" role="tab" aria-controls="specimens-tab" aria-selected="false">Ejemplares</a>
                                </li>
                            </ul>
                        </div><!-- card header -->
                        <div class="card-body">
                            <div class="tab-content" id="tabContent">
                                <div class="tab-pane fade show active" id="specimens-tab" role="tabpanel" aria-labelledby="breeders-specimens-tab">
                                ...
                                <div id="div_header01">
                                    ...
                                    <table id="table01" class="table table-striped table-bordered shadow-lg mt-4" style="width:100%">
                                        ...
                                        <tbody>
                                            @foreach ($specimens_breeder as $specimen)
                                                <tr>
